# 8.2 Main Menu, We will change the scene with the game if we press SPACE buttons on main menu
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE:
                    waiting = False
                    isAlive = True  # Set isAlive to True to start the game
...
# Note: There is a problem now, when the player dies, the program shows the main menu but after then if you press space, it will not start the game again.
# - To fix this, we need to reset the game state when the player dies.