# 10, Final Responsive Dino Run Game
...
game_speed = 5 * ratio_x  # Speed of the game, can be adjusted for difficulty
...
ground_image = pygame.transform.scale(ground_image, (WIDTH, abs(60 *  ratio_y)))
...
class Ground(pygame.sprite.Sprite):
    def __init__(self, x):
        ...
        self.rect.left = x
        self.rect.y = HEIGHT - 60 * ratio_y  # Adjust ground position based on ratio
...
class Player():
    ...
    def move(self):
        ...
        if keys[pygame.K_DOWN]:
            if self.Inair:
                self.gravity = 5 * ratio_y  # Increase gravity when ducking in air
            else:
                self.ducking = True
        else:
            self.ducking = False
        ...
        for ground in ground_group:
            if self.rect.colliderect(ground.rect) and dy >= 0:
                ...
                self.velocity_y = 0
                self.Inair = False
                self.gravity = 0.5 * ratio_y  # Reset gravity when on ground
...
while running:
    ...
    if not isAlive:
        ...
        game_speed = 5 * ratio_x
        if score > high_score:
            save_high_score(score)  # Save high score if current score is higher
    player.draw()

    increment_game_speed()  # Increment game speed over time
